...
function renderLanguage(){
  ...
  const paint=()=>{const w=words[index%words.length];card.innerHTML=`<div class="flash-meta"><span>EN → ${lang==='en'?'DEFINITION':'RU'}</span><span>${index+1}/${words.length}</span></div><div><div class="word">${w[0]}</div><div class="translation">${lang==='en'?w[2]:w[1]}</div></div><div class="flash-meta"><span>↔ SWIPE</span><span>SEOCRY WORDS</span></div>`;card.style.transform='';card.style.opacity='1';};
